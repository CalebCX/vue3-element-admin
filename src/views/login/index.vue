  ...
  setup() {
    ...
    const state = reactive({
      ...
      submit: () => {
        ...
        formData.append('userName', state.model.userName)
        formData.append('password', state.model.password)
        formData.append('captcha', state.model.captcha) // 添加验证码字段
        formData.append('codeKey', state.model.codeKey) // 添加验证码key字段
        state.loginForm.validate(async valid => {
          if (valid) {
            state.loading = true
